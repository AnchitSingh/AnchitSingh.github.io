...
function update() {
    translateY += (targetTranslateY - translateY + snapBack) * easeOutQuad(0.1);
    content.style.transform = "translateY(" + translateY + "px)";

    // Limit the amount of scrolling
    if (translateY > 0) {
        snapBack = -translateY * 0.1;
        if (Math.abs(snapBack) < 0.1) {
            snapBack = 0;
            targetTranslateY = 0;
        }
    } else if (translateY < -contentHeight + window.innerHeight) {
        snapBack = (-contentHeight + window.innerHeight - translateY) * 0.1;
        if (Math.abs(snapBack) < 0.1) {
            snapBack = 0;
